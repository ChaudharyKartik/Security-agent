...
| A scan shows `status: error` right after restarting the API, with "interrupted by a server restart" in the error message | Expected, not a bug — that scan was still running when the server stopped (crash, redeploy, `docker compose down`), so there was no way to resume it. The API marks it failed on startup instead of leaving it stuck at its last in-progress status forever. Just start a new scan against the same target. |
