"""
Validator Module
Human-in-the-loop validation: approve, reject (false positive), escalate.
"""
import logging
...
def validate_finding(finding: dict, action: str, validator_name: str,
    ...
    logger.info(f"[VALIDATOR] [{finding.get('id', '?')}] Action: {action} by {validator_name}")
    return finding
...
def validate_batch(findings: list, approved_ids: list, rejected_ids: list,
    ...
    for finding in findings:
        ...
        if fid in approved_ids:
            validate_finding(finding, "approve", validator_name)
        elif fid in rejected_ids:
            validate_finding(finding, "reject", validator_name)
    return findings
